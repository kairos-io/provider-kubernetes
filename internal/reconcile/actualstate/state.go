...
type State struct {
	...
	// APIServerReachable reports whether the LOCAL apiserver answers /healthz
	// (ADR-12-R1). After an A/B image swap the new kubelet can crashloop on a stale
	// kubeadm-flags.env, leaving the control plane down; when an upgrade is due and
	// this is false, the kubelet config must be repaired (regenerated by the new
	// kubeadm) before `kubeadm upgrade apply` can reach the control plane. On an
	// Initialized node with no upgrade due, false means the control plane this
	// node runs is not serving, which must not be reported as converged.
	APIServerReachable bool
	// InitIncomplete reports that this node's `kubeadm init` stopped before its
	// kubelet-finalize phase: kubelet.conf still embeds the client certificate
	// kubeadm wrote at init, although the kubelet's rotated certificate exists,
	// so the phase that would have rewritten it never ran. The phases before it
	// (upload-config, upload-certs, mark-control-plane, bootstrap-token) create
	// the objects joins depend on, so they may be missing. Only meaningful for
	// an Initialized node; false whenever the files do not allow a decision.
	InitIncomplete bool
}
